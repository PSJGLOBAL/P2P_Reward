...
public:
    ...
    bool bRead_ResultInfo(const std::string _ResultInfo_Json, const std::string _Master_Wallet_Addr, const std::string _Master_Wallet_PrivateKey, float& _nTotalRewardCount);
    bool bRead_RewardResultInfo(const std::string _RewardResultInfoJson);

    bool bWrite_RewardResult(std::map<int, StReward_Result> _ResultMap, std::string FaileName, float nTotalRewardCoin);
